...
int32 SAnimatedSwitcher::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
...
void SAnimatedSwitcher::TransitionToIndex(int32 NewWidgetIndex, bool bInstantTransition)
{
	// Cache the widget we want to reach
	PendingActiveWidget = GetWidget(NewWidgetIndex);
	if (!PendingActiveWidget.IsValid())
	{
		UE_LOG(LogSlate, Verbose, TEXT("Called SAnimatedSwitcher::TransitionToIndex('%d') to an invalid Index"), NewWidgetIndex);
		return;
	}
	...
	else if (bNewGoalHigher || bNewGoalLower)
	{
		...
		if (!bIsTransitionTimerRegistered)
		{
			...
			RegisterActiveTimer(0.f, FWidgetActiveTimerDelegate::CreateSP(this, &SAnimatedSwitcher::UpdateTransition));
		}
	}
}
...
void SAnimatedSwitcher::SetTransition(float Duration, ETransitionCurve Curve)
{
	TransitionSequence = FCurveSequence(0.f, Duration * 0.5f, TransitionCurveToCurveEaseFunction(Curve));
}

EActiveTimerReturnType SAnimatedSwitcher::UpdateTransition(double InCurrentTime, float InDeltaTime)
{
	...
	if (!TransitionSequence.IsPlaying())
	{
		...
		else
		{
			...
			{
				UE_LOG(LogSlate, Verbose, TEXT("SAnimatedSwitcher Pending Widget became invalid while a transition was happening"));
			}
			PendingActiveWidget = nullptr;
		}
	}
...
float SAnimatedSwitcher::GetTransitionProgress() const
{ 
...
bool SAnimatedSwitcher::IsTransitionPlaying() const
{
